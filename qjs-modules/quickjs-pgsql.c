...
js_pgconn_print_values(JSContext* ctx, PGSQLConnection* pq, DynBuf* out, JSValueConst values) {
  ...
  /*if(!JS_IsUndefined(iter)) {
    ...
    if(JS_GetOwnPropertyNames(ctx, &tmp_tab, &tmp_len, values, JS_GPN_STRING_MASK |
  JS_GPN_ENUM_ONLY)) { JS_FreeValue(ctx, iter); JS_ThrowTypeError(ctx, "argument is must be an
  object"); return;
    }
  ...
  switch(magic) {
    ...
    case PROP_CLIENT_ENCODING: {
      ...
      if((charset = JS_ToCString(ctx, value))) {
        int ret = PQsetClientEncoding(pq->conn, charset);
